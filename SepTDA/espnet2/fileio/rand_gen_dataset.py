"""Datasets that generate random arrays on the fly from a shape file.

These are used in place of a real feature/label reader (e.g. for dummy
inputs, testing, or padding) when only the *shape* of the data matters.
The shape file is a 2-column Kaldi-style text file where the value is a
comma-separated shape, e.g.::

    shape.txt:
        uttA 123,83
        uttB 34,83

Indexing the dataset with a key generates a fresh random array of the
corresponding shape (a new array on every access -- nothing is cached).
"""
...
class FloatRandomGenerateDataset(collections.abc.Mapping):
    ...
    def __init__(
        ...
        self.utt2shape: Dict[str, List[int]] = load_num_sequence_text(
