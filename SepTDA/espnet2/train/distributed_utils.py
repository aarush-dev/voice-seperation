"""Distributed-training setup helpers.

Resolves rank/world-size/master-address configuration from either explicit
CLI flags, environment variables (``RANK``, ``WORLD_SIZE``, ``LOCAL_RANK``,
``MASTER_ADDR``, ``MASTER_PORT``), or a launcher (currently Slurm's
``srun``, or mpi4py for node-rank discovery under
``multiprocessing_distributed``), and initializes
...
class DistributedOption:
    ...
    Call :meth:`init_options` once (after parsing CLI args, with
    :func:`resolve_distributed_mode` having already set ``distributed``) to
    fill in rank/world-size/master-address from CLI/environment/launcher,
    then :meth:`init_torch_distributed` to actually initialize the PyTorch
    process group.
    """
    ...
    def init_options(self) -> None:
        """Resolve rank/world-size/local-rank/master-address from CLI/env/launcher.

        No-op when ``self.distributed`` is False. Mutates this instance's
        fields in place, and rewrites ``dist_init_method`` to
        ``tcp://{addr}:{port}`` when using ``"env://"`` init.
        """
        if self.distributed:
    ...
    def init_torch_distributed(self) -> None:
        """Call ``torch.distributed.init_process_group`` using the resolved options.

        No-op when ``self.distributed`` is False. Also pins the current
        process to ``self.local_rank``'s CUDA device when running one
        process per GPU.
        """
        if self.distributed:
            # See:
...
def resolve_distributed_mode(args: "argparse.Namespace") -> None:
    """Decide (and set) ``args.distributed`` from ``args.ngpu``/world size/launcher.

    Note that ``args.distributed`` is set by only this function -- it is
    not a real ArgumentParser option. Also normalizes
    ``args.multiprocessing_distributed`` and ``args.local_rank`` for the
    single-GPU-or-CPU case, and validates that the required rank
    information is available for the chosen mode.
    """
...
def is_in_slurm_job() -> bool:
    """Whether this process appears to have been launched by Slurm."""
    return "SLURM_PROCID" in os.environ and "SLURM_NTASKS" in os.environ


def is_in_slurm_step() -> bool:
    """Whether this process is inside an active Slurm step (``srun``)."""
    return (
        is_in_slurm_job()
...
def _int_or_none(x: Optional[str]) -> Optional[int]:
    """Cast ``x`` to int, passing through None."""
    if x is None:
        return x
    return int(x)
...
def get_rank(prior=None, launcher: str = None) -> Optional[int]:
    """Resolve the global rank: ``prior`` > launcher-provided > ``$RANK`` > None."""
    if prior is None:
        if launcher == "slurm":
...
def get_world_size(prior=None, launcher: str = None) -> int:
    """Resolve the world size: ``prior`` > launcher-provided > ``$WORLD_SIZE`` > 1."""
    if prior is None:
        if launcher == "slurm":
            if not is_in_slurm_step():
                raise RuntimeError("This process seems not to be launched by 'srun'")
...
def get_local_rank(prior=None, launcher: str = None) -> Optional[int]:
    """Resolve the local rank (same as the GPU device id for this process).

    Priority: ``prior`` > launcher-provided > ``$LOCAL_RANK`` > (inferred
    from a single-id ``$CUDA_VISIBLE_DEVICES``, which is then popped from
...
def get_master_addr(prior=None, launcher: str = None) -> Optional[str]:
    """Resolve the master address: ``prior`` > first node in Slurm's nodelist > ``$MASTER_ADDR``."""
    if prior is None:
        if launcher == "slurm":
            if not is_in_slurm_step():
                raise RuntimeError("This process seems not to be launched by 'srun'")
...
def get_master_port(prior=None) -> Optional[int]:
    """Resolve the master port: ``prior`` > ``$MASTER_PORT``."""
    if prior is not None:
        return prior
    else:
