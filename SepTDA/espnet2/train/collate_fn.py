"""Collate function module.

A "collate_fn" turns a list of per-utterance ``(uid, {field: array})``
samples (as produced by :class:`espnet2.train.dataset.ESPnetDataset`) into
one batch: field arrays are zero/pad-value-padded to the batch's max
length and stacked into ``torch.Tensor``\\ s, with a ``{field}_lengths``
tensor added alongside every padded (sequence) field.
"""
import math
...
class CommonCollateFn:
    ...
    Kept as a class (rather than a bare function) so it can be constructed
    once with fixed pad values / non-sequence field names and then reused
    as the ``collate_fn`` for a PyTorch DataLoader.
    """
    ...
    def __call__(
        ...
        """Pad and stack one batch's worth of per-utterance samples."""
...
class HuBERTCollateFn(CommonCollateFn):
    ...
    def __call__(
        ...
        """Crop each (waveform, label) pair to a common length, then pad and stack."""
