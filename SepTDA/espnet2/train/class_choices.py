"""ClassChoices module: CLI-driven registry for pluggable component classes."""
import argparse
...
class ClassChoices:
    ...
    def choices(self) -> Tuple[Optional[str], ...]:
        """Return the valid ``--{name}`` argparse choices (plus None, if optional)."""
    ...
    def get_class(self, name: Optional[str]) -> Optional[type]:
        """Look up the class registered under ``name`` (case-insensitive).

        Returns None if ``name`` is None (or "none"/"null"/"nil") and this
        choice is optional; raises ValueError for any other unknown name.
        """
        assert check_argument_types()
    ...
    def add_arguments(self, parser: argparse.ArgumentParser) -> None:
        """Register ``--{name}`` (the choice) and ``--{name}_conf`` (its kwargs)."""
        parser.add_argument(
            f"--{self.name}",
