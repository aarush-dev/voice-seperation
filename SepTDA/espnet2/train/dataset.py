"""Map-style dataset module.

:class:`ESPnetDataset` is a ``torch.utils.data.Dataset`` that lazily loads
one or more parallel "scp"-style files (audio/text/features/...), keyed by
utterance id, and merges them into a single ``{data_name: array}`` dict per
utterance -- optionally passing it through a
:class:`espnet2.train.preprocessor.AbsPreprocessor` before returning.
``DATA_TYPES`` maps each supported ``--*_type`` (e.g. "sound", "npy",
"text_int") to the loader factory function used to open its file, plus the
extra constructor kwargs (if any) that loader accepts.
"""
import collections
import copy
import functools
...
class AdapterForSoundScpReader(collections.abc.Mapping):
    """Wraps a sound/kaldi-ark scp reader so ``__getitem__`` returns a bare ndarray.

    The underlying reader returns ``(rate, array)`` (or ``(array, rate)``
    for the extended ark format); this adapter strips the sample rate
    (after checking it's consistent across the whole file) and optionally
    casts to ``dtype``.
    """
...
class H5FileWrapper:
    """Read-only ``Mapping``-like view over the top level of an HDF5 file."""
...
class AdapterForSingingScoreScpReader(collections.abc.Mapping):
    """Wraps a :class:`SingingScoreReader` to return ``(tempo, notes)`` per key."""
...
class AdapterForLabelScpReader(collections.abc.Mapping):
    """Wraps a label reader to return ``(start/end times, phone labels)`` per key."""
...
def sound_loader(path, float_dtype=None, multi_columns=False):
    """Build a loader over a ``wav.scp``-style file, returning bare ndarrays."""
...
def multi_columns_sound_loader(path, float_dtype=None):
    """Like :func:`sound_loader`, but each scp line may list multiple audio files
    (one per channel) to be loaded together as a multi-channel signal."""
    return sound_loader(path, float_dtype, multi_columns=True)


def score_loader(path):
    """Build a loader returning ``(tempo, notes)`` per utterance from a score scp."""
    loader = SingingScoreReader(fname=path)
    return AdapterForSingingScoreScpReader(loader)


def label_loader(path):
    """Build a loader returning ``(timestamps, phone labels)`` per utterance."""
    loader = read_label(path)
    return AdapterForLabelScpReader(loader)


def kaldi_loader(path, float_dtype=None, max_cache_fd: int = 0):
    """Build a loader over a Kaldi ``.ark``/``.scp`` file, returning bare ndarrays."""
...
def rand_int_loader(filepath, loader_type):
    """Build a loader generating random ints in ``[low, high]`` per the shapes file.

    ``loader_type`` encodes the bounds, e.g. "rand_int_3_10" -> [3, 10].
    """
    # e.g. rand_int_3_10
    try:
...
class AbsDataset(Dataset, ABC):
    """Base class for map-style ESPnet2 datasets keyed by utterance id."""
    ...
    def __getitem__(self, uid) -> Tuple[Any, Dict[str, np.ndarray]]:
        """Return ``(uid, {field_name: array})`` for one utterance."""
        raise NotImplementedError
...
class ESPnetDataset(AbsDataset):
    ...
    a per-utterance loader is built for ``path`` per ``loader_type`` (see
    ``DATA_TYPES``) and exposed under ``data_name``. ``__getitem__`` loads
    the matching entry from every loader, runs the optional ``preprocess``
    callable over the merged dict, casts everything to
    ``float_dtype``/``int_dtype``, and (if ``max_cache_size > 0``) caches
    the result.
    ...
    def __iter__(self):
        """Iterate over utterance ids, per the first loader's key order."""
    ...
    def _resolve_uid(self, uid: Union[str, int]) -> str:
        """Map an integer index to its string utterance id, per the first loader."""
        if isinstance(uid, int):
            d = next(iter(self.loader_dict.values()))
            uid = list(d)[uid]
        return uid

    def _load_raw_fields(self, uid: str) -> Dict[str, Union[np.ndarray, torch.Tensor]]:
        """Fetch ``uid``'s entry from every loader, normalizing list/Tensor/Number values."""
        data = {}
    ...
    def __getitem__(self, uid: Union[str, int]) -> Tuple[str, Dict[str, np.ndarray]]:
        """Load, preprocess, and dtype-cast one utterance's data (with caching)."""
        assert check_argument_types()
        ...
        data = self._load_raw_fields(uid)
