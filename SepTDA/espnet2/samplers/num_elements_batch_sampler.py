"""Batch sampler that targets a constant total tensor-element ("bins") budget.

Like :class:`~espnet2.samplers.length_batch_sampler.LengthBatchSampler`,
this greedily accumulates length-sorted samples into a batch until a bin
budget is exceeded, but the bin count here is the total number of tensor
elements (length x feature dimensions) rather than just summed length.
This matters when the feature dimensionality varies across samples or
shape files, e.g. multi-channel or multi-speaker features. See the other
sibling samplers (folded, sorted, unsorted) for batch-size strategies that
don't target an explicit bin budget.
"""
...
class NumElementsBatchSampler(AbsSampler):
    """Batch sampler with a variable batch size driven by an element-count budget.
    ...
    - ``padding=True``: ``batch_size * max_length * feat_dim`` per shape
      file (as if every sequence were padded to the longest one currently
      in the batch), summed over shape files. This requires the feature
      dimension to be identical for every sample in a given shape file.
    - ``padding=False``: the sum of ``prod(shape)`` over all samples and
      shape files, i.e. the true (unpadded) element count.
    """
    ...
    def __init__(
        ...
        if padding:
            ...
            feat_dims = [np.prod(shape[keys[0]][1:]) for shape in utt2shapes]
        else:
            feat_dims = None
    ...
    def _decide_batch_sizes(
        ...
        for key in keys:
            ...
            if padding:
                for shape, shape_file in zip(utt2shapes, shape_files):
                    if tuple(shape[key][1:]) != tuple(shape[keys[0]][1:]):
                        raise RuntimeError(
                            "If padding=True, the "
                            f"feature dimension must be unified: {shape_file}",
                        )
                bins = sum(
                    len(current_batch_keys) * shape[key][0] * feat_dim
                    for shape, feat_dim in zip(utt2shapes, feat_dims)
                )
            else:
                bins = sum(
                    np.prod(shape[k])
                    for k in current_batch_keys
                    for shape in utt2shapes
                )
