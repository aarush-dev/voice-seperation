"""Factory for constructing a concrete batch sampler from a config-style name.

Training configs (YAML) select a batch-sampling strategy by a short string
key such as "unsorted", "sorted", "folded", "numel" or "length". This module
maps those keys to the corresponding ``AbsSampler`` subclasses and also
exposes human-readable descriptions of each strategy via ``BATCH_TYPES``,
which is used to build the "--batch_type" CLI help text.
"""
...
def build_batch_sampler(
    ...
    Dispatches on ``type`` to construct and return the matching
    ``AbsSampler`` subclass, forwarding the relevant subset of arguments to
    its constructor.
    ...
    Args:
        ...
        padding: Whether sequences are input as a padded tensor or not.
            used for "numel" mode
        utt2category_file: Optional file mapping each key to a category;
            when given, mini-batches are formed within each category.
        remove_samples_with_speaker_overlap: Used for "folded" mode; drop
            samples whose speaker ids repeat within the same mixture.

    Returns:
        An instantiated ``AbsSampler`` subclass matching ``type``.

    Raises:
        ValueError: If ``shape_files`` is empty, ``type`` is unrecognized,
            or (for "folded") ``fold_lengths`` doesn't match ``shape_files``
            in length.
    """
