...
"""SincNet-style learnable band-pass filters.

Implements the Sinc convolution front-end from SincNet, which learns a
small set of band-pass filter cutoffs (rather than arbitrary FIR taps) and
applies them as a time-domain convolution. Used as an alternative,
parameter-efficient waveform front-end ahead of the encoder, typically
paired with :class:`LogCompression` as its activation.
"""
import math
...
class SincConv(torch.nn.Module):
    ...
    def __init__(
        ...
        # self.f holds the learnable (low, high) cutoff pairs per output
        # channel, in normalized frequency; set by init_filters() below.
        self.f = None
    ...
    def _create_filters(self, device: torch.device) -> None:
        """Calculate coefficients.

        This function (re-)calculates the filter convolutions coefficients.
        """
        # f_mins, f_maxs: (out_channels,) band-pass edge frequencies
        ...
        # filters: (out_channels, 1, kernel_size), ready for conv1d weight
...
class MelScale:
    ...
    @staticmethod
    def convert(f: torch.Tensor) -> torch.Tensor:
        """Convert Hz to mel."""
