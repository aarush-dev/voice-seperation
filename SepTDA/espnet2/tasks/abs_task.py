...
Defines :class:`AbsTask`, the base class every ESPnet2 task (ASR, TTS, Enh,
SepTDA, ...) subclasses to get a full training/inference CLI almost for
free. A concrete task mainly needs to:

- declare ``class_choices_list``, a list of
  :class:`~espnet2.train.class_choices.ClassChoices` registries. Each
  registry maps short string keys (e.g. ``"stft"``) to concrete component
  classes (e.g. an encoder implementation); ``ClassChoices.add_arguments``
  exposes that mapping as a ``--<name>``/``--<name>_conf`` argparse option
  pair so a YAML config can select and configure an implementation purely
  by name. These key strings are part of the public config format and must
  not be renamed.
- implement :meth:`AbsTask.add_task_arguments`, :meth:`AbsTask.build_model`,
  :meth:`AbsTask.build_collate_fn`, :meth:`AbsTask.build_preprocess_fn`,
  :meth:`AbsTask.required_data_names`, and
  :meth:`AbsTask.optional_data_names`.

``AbsTask`` itself then provides argument parsing (:meth:`get_parser`),
process/distributed-training bootstrapping (:meth:`main`,
:meth:`main_worker`), optimizer/scheduler construction
(:meth:`build_optimizers`), and the iterator-factory machinery
(:meth:`build_iter_factory` and friends) that turns raw scp files into
mini-batches for the :class:`~espnet2.train.trainer.Trainer`.
"""
import argparse
import functools
import logging
...
class IteratorOptions:
    """Mode-resolved (train/valid/plot_att) settings for building one iterator factory.

    Built by :meth:`AbsTask.build_iter_options` (or a task-specific
    override) and consumed by ``build_sequence_iter_factory``/
    ``build_chunk_iter_factory``/``build_task_iter_factory``.
    """
...
class AbsTask(ABC):
    """Base class for an ESPnet2 task: wires argument parsing, model/data building, and training.

    Never instantiated: every method is a ``@classmethod`` (or
    ``@staticmethod``) so the whole class effectively acts as a namespace,
    which keeps a single "God object" instance from accumulating state.
    Subclasses (e.g. ``EnhancementTask``,
    ``TargetSpeakerExtractionAndEnhancementTask``) fill in
    ``class_choices_list`` and the abstract methods below; everything else
    here (``get_parser``, ``main``, ``main_worker``, the
    ``build_*_iter_factory`` family, ...) is shared infrastructure.
    """
    ...
    def add_task_arguments(cls, parser: argparse.ArgumentParser) -> None:
        """Register this task's own CLI/YAML arguments on top of the common ones from ``get_parser``.

        Typically ends with a loop over ``cls.class_choices_list`` calling
        ``class_choices.add_arguments(group)`` so each registry contributes
        its ``--<name>``/``--<name>_conf`` option pair.
        """
        pass
    ...
    def build_preprocess_fn(
        ...
        """Return the (optional) preprocessing function applied to each raw sample before collation.

        Return ``None`` if this task doesn't need one.
        """
        raise NotImplementedError
    ...
    def build_model(cls, args: argparse.Namespace) -> AbsESPnetModel:
        """Instantiate the task's model from parsed args (typically resolving each sub-module via a ``ClassChoices`` registry)."""
        raise NotImplementedError

    @classmethod
    def get_parser(cls) -> config_argparse.ArgumentParser:
        """Build the full argparse parser for this task (common + task-specific options).

        Assembles the generic ESPnet2 training options (grouped into the
        ``_add_*_arguments`` helpers below), the trainer's own options via
        ``cls.trainer.add_arguments``, and finally the task-specific options
        via ``cls.add_task_arguments`` (which also registers the ``--<name>``/
        ``--<name>_conf`` pair for every ``ClassChoices`` in
        ``cls.class_choices_list``).
        """
        ...
        cls._add_distributed_arguments(parser)
        cls._add_cudnn_arguments(parser)
        cls._add_collect_stats_arguments(parser)
        cls._add_trainer_arguments(parser)
        cls._add_pretrain_arguments(parser)
        cls._add_batch_sampler_arguments(parser)
        cls._add_sequence_iterator_arguments(parser)
        cls._add_chunk_iterator_arguments(parser)
        cls._add_dataset_arguments(parser)
        cls._add_optimizer_arguments(parser)
    ...
    def _add_common_arguments(cls, parser: argparse.ArgumentParser) -> None:
        """Common configuration: logging, dry-run, device count, iterator type."""
    ...
    @classmethod
    def _add_distributed_arguments(cls, parser: argparse.ArgumentParser) -> None:
        """Distributed-training (torch.distributed) related options."""
    ...
    @classmethod
    def _add_cudnn_arguments(cls, parser: argparse.ArgumentParser) -> None:
        """cuDNN backend flags."""
    ...
    @classmethod
    def _add_collect_stats_arguments(cls, parser: argparse.ArgumentParser) -> None:
        """"collect stats" mode related options."""
    ...
    @classmethod
    def _add_trainer_arguments(cls, parser: argparse.ArgumentParser) -> None:
        """Trainer related options: epochs, early stopping, logging backends."""
    ...
    @classmethod
    def _add_pretrain_arguments(cls, parser: argparse.ArgumentParser) -> None:
        """Pretrained-parameter loading / freezing options."""
    ...
    @classmethod
    def _add_batch_sampler_arguments(cls, parser: argparse.ArgumentParser) -> None:
        """Generic batch-size/iteration-count options shared by all batch samplers."""
    ...
    @classmethod
    def _add_sequence_iterator_arguments(cls, parser: argparse.ArgumentParser) -> None:
        """Options specific to the "sequence" iterator (length/bin-based batch samplers)."""
    ...
    @classmethod
    def _add_chunk_iterator_arguments(cls, parser: argparse.ArgumentParser) -> None:
        """Options specific to the "chunk" iterator (fixed-length audio chunks)."""
    ...
    @classmethod
    def _add_dataset_arguments(cls, parser: argparse.ArgumentParser) -> None:
        """Dataset-loading options: --*_data_path_and_name_and_type, caching."""
    ...
    @classmethod
    def _add_optimizer_arguments(cls, parser: argparse.ArgumentParser) -> None:
        """Optimizer/scheduler selection and weight-decay-exclusion options."""
    ...
    def build_optimizers(
        ...
        """Build the (single, by default) optimizer selected by ``--optim``/``--optim_conf``.

        Must be overridden by subclasses that set ``num_optimizers != 1``.
        """
    ...
    @classmethod
    def check_required_command_args(cls, args: argparse.Namespace) -> None:
        """Exit with a usage error if any argument in ``args.required`` (e.g. ``output_dir``) is still ``None``."""
        assert check_argument_types()
        for k in vars(args):
            if "-" in k:
    ...
    def print_config(cls, file=sys.stdout) -> None:
        """Write the fully-resolved default config (all args + their class-specific defaults) as YAML."""
        assert check_argument_types()
    ...
    def main(
        ...
        """CLI entry point: parse args (if not given), then dispatch to one or many ``main_worker`` processes.

        For single-GPU/CPU or torchrun-style launches, runs ``main_worker``
        directly in the current process. For
        ``--multiprocessing_distributed``, spawns one child process per GPU
        and waits for them to join, mirroring PyTorch's imagenet example.
        """
        assert check_argument_types()
    ...
    @classmethod
    def main_worker(cls, args: argparse.Namespace) -> None:
        """Run a single training/collect-stats worker process.

        Called by :meth:`main` directly for a single-process run, or once
        per spawned child process under ``--multiprocessing_distributed``.
        Steps, in order: initialize the distributed process group and
        logging, build the model/optimizers/schedulers (skipped if
        collect_stats doesn't need a forward pass), dump the resolved
        config to ``config.yaml``, then run one of: dry run (build only),
        "collect stats" (derive feature stats and exit), or full training
        via ``cls.trainer.run``.
        """
        assert check_argument_types()
        ...
        cls._setup_logging(distributed_option, args.log_level)
        ...
        # 2-4. Build model, optimizers, and schedulers.
        model, optimizers, schedulers = cls._build_model_optimizers_schedulers(args)
        ...
        output_dir = cls._dump_config_file(args, distributed_option)

        if args.dry_run:
            pass
        elif args.collect_stats:
            cls._run_collect_stats(args, model, output_dir)
        else:
            # 6. Loads pre-trained model
            cls._load_pretrained_params(args, model)
            ...
            ) = cls._build_training_iterators(args, distributed_option)

            # 8. Start training
            cls._init_wandb(args, output_dir, distributed_option)
            cls._run_training(
                args,
                model,
                optimizers,
    ...
    @classmethod
    def _setup_logging(
        cls, distributed_option: DistributedOption, log_level: str
    ) -> None:
        """Configure the root logger, silencing non-rank-0 processes under distributed training.

        NOTE(kamo): logging.basicConfig() is invoked here (instead of in
        main()) because it can be invoked only once per process.
        """
    ...
    @classmethod
    def _build_model_optimizers_schedulers(
        cls, args: argparse.Namespace
    ) -> Tuple[
        Optional[AbsESPnetModel],
        Optional[List[torch.optim.Optimizer]],
        Optional[List[Any]],
    ]:
        """Build the model plus its optimizers/schedulers.

        Returns ``(None, None, None)`` when ``--collect_stats`` is set and
        the model config says feature extraction doesn't require a forward
        pass (``extract_feats_in_collect_stats=False``), since building the
        model would then be wasted work.
        """
        if (
            args.collect_stats
    ...
    @classmethod
    def _dump_config_file(
        cls, args: argparse.Namespace, distributed_option: DistributedOption
    ) -> Path:
        """Create --output_dir and, on rank 0, dump the resolved args as config.yaml."""
    ...
    @classmethod
    def _run_collect_stats(
        cls, args: argparse.Namespace, model: Optional[AbsESPnetModel], output_dir: Path
    ) -> None:
        """Run "collect stats" mode: derive per-key shapes/dims and feature statistics, then exit.

        This mode has two roles:
    ...
    @classmethod
    def _load_pretrained_params(
        cls, args: argparse.Namespace, model: AbsESPnetModel
    ) -> None:
        """Load each ``--init_param`` entry into ``model``.

        See the ``--init_param`` help text for the
        ``file_path:src_key:dst_key:exclude_keys`` format.
        """
        # 6. Loads pre-trained model
    ...
    @classmethod
    def _build_training_iterators(
        cls, args: argparse.Namespace, distributed_option: DistributedOption
    ) -> Tuple[AbsIterFactory, AbsIterFactory, Optional[AbsIterFactory]]:
        """Build the train/valid/attention-plot iterator factories for a full training run."""
        # 7. Build iterator factories
        ...
        return train_iter_factory, valid_iter_factory, plot_attention_iter_factory
    ...
    def _init_wandb(
        ...
        """Log in to and initialize wandb, if ``--use_wandb`` is set; otherwise a no-op.

        May flip ``args.use_wandb`` back to ``False`` if wandb isn't
        configured on this machine, or on non-rank-0 processes under
        distributed training (wandb only needs one run logging the
        aggregated data, so rank 0 is enough).
        """
        # 8. Start training
        if args.use_wandb:
            if wandb is None:
    ...
    def _run_training(
        ...
        """Hand off to ``cls.trainer.run`` and finalize the wandb run afterwards."""
    ...
    def build_iter_options(
        ...
        ``mode`` is one of "train", "valid", or "plot_att" (the small
        held-out set used to render attention plots each epoch); each is
        delegated to a ``_<mode>_iter_options`` helper below.
        """
        if mode == "train":
    ...
    def build_sequence_iter_factory(
        ...
        driven by the shape files) and are sharded across ranks for
    ...
    def build_chunk_iter_factory(
        ...
        """Build a :class:`ChunkIterFactory` that yields fixed-length chunks of audio.
        ...
        chunk iterator itself) and sharded across ranks for distributed
        training.
        """
        assert check_argument_types()
    ...
    def build_multiple_iter_factory(
        ...
        """Build a :class:`MultipleIterFactory` over data pre-split into ``num_splits`` shards.

        Used with ``--multiple_iterator true``, e.g. when a single-node
        machine can't hold the whole dataset's shape/cache info in memory:
        each of ``data_path_and_name_and_type``/``shape_files`` must be a
        directory containing a ``num_splits`` file and ``split.<i>``
        sub-paths, and a lazily-built ``build_iter_factory`` is created for
        each split.
        """
        assert check_argument_types()
        iter_options = cls.build_iter_options(args, distributed_option, mode)
