...
Implements Griffin-Lim phase reconstruction for synthesizing a waveform
from a magnitude (or log-Mel) spectrogram, when no learned vocoder is
available. `logmel2linear` inverts a log-Mel filterbank back to a linear
magnitude spectrogram; `griffin_lim` then iteratively estimates phase to
reconstruct the waveform; `Spectrogram2Waveform` bundles both steps
behind a single callable.
"""
...
class Spectrogram2Waveform(object):
    ...
    def __repr__(self) -> str:
        """Return a human-readable summary of the configured parameters."""
