"""argparse ``type=`` converters for CLI/YAML config parsing.

Each function here is used as the ``type=`` callable for an
``argparse.add_argument(...)`` call, or invoked directly on a config
string. Their accepted input strings, return values, and error
conditions are part of the CLI/config contract, so parsing logic in
...
def str2bool(value: str) -> bool:
    """Parse a boolean-ish string (e.g. "yes"/"no", "true"/"false", "1"/"0").

    Delegates to `distutils.util.strtobool`; raises ``ValueError`` for
...
def remove_parenthesis(value: str) -> str:
    """Strip surrounding whitespace, then a single matching pair of
    ``(...)`` or ``[...]`` if the (stripped) string starts and ends with
    one of those pairs."""
    value = value.strip()
...
def remove_quotes(value: str) -> str:
    """Strip surrounding whitespace, then a single matching pair of
    ``"..."`` or ``'...'`` quotes if the (stripped) string starts and
    ends with one of those pairs."""
    value = value.strip()
    if value.startswith('"') and value.endswith('"'):
        value = value[1:-1]
    elif value.startswith("'") and value.endswith("'"):
...
def humanfriendly_parse_size_or_none(value: str) -> Optional[float]:
    """Parse a human-friendly size string (e.g. "1.5 GB") to a byte count,
    or ``None`` for "none"/"null"/"nil"."""
    if value.strip().lower() in ("none", "null", "nil"):
        return None
...
def str_or_int(value: str) -> Union[str, int]:
    """Parse ``value`` as an int if possible, otherwise return it unchanged as a string."""
    try:
        return int(value)
    except ValueError:
        return value
