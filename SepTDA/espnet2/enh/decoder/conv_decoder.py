"""Learned 1-D transposed-convolutional decoder (as used in e.g. Conv-TasNet)."""
...
class ConvDecoder(AbsDecoder):
    ...
    Inverts a :class:`~espnet2.enh.encoder.conv_encoder.ConvEncoder` with a
    single strided 1-D transposed convolution (no bias, no nonlinearity),
    reconstructing a waveform from the per-speaker encoder-domain features.
    """
    ...
    def forward(
        ...
        """
        # (Batch, T, F) -> (Batch, F, T): channel-first layout for ConvTranspose1d.
    ...
    def streaming_merge(
        ...
        # Overlap-add: each chunk contributes to its own [start, start+frame_size)
        # window of the output; overlapping regions are summed.
