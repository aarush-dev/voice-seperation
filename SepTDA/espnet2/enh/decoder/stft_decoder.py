"""STFT-based decoder: maps a complex time-frequency spectrum back to a waveform."""
...
class STFTDecoder(AbsDecoder):
    ...
    Inverts a :class:`~espnet2.enh.encoder.stft_encoder.STFTEncoder`: first
    undoes the encoder's magnitude-compression "spec transform" (see
    :meth:`spec_back`), then applies the inverse STFT to reconstruct a
    waveform.
    """
    ...
    def forward(
        self, input: ComplexTensor, ilens: torch.Tensor, fs: int = None
        ...
        # Restore self.stft's window/hop/n_fft to their configured (default_fs)
        # values, undoing any temporary _reconfig_for_fs(fs) done above.
        self._reset_config()
        return wav, wav_lens
    ...
    def _reconfig_for_fs(self, fs: int) -> None:
    ...
    def _get_window_func(self) -> torch.Tensor:
        """Build the (unpadded) analysis/synthesis window of length ``win_length``."""
        window_func = getattr(torch, f"{self.window}_window")
        window = window_func(self.win_length)
        return window

    def spec_back(self, spec: ComplexTensor) -> ComplexTensor:
        """Invert the encoder's magnitude-compression "spec transform".

        This is the exact inverse of
        :meth:`espnet2.enh.encoder.stft_encoder.STFTEncoder.spec_transform_func`,
        selected by the same ``self.spec_transform_type``:

        - ``"exponent"``: undo the ``spec_factor`` scaling, then raise the
          magnitude to ``1 / spec_abs_exponent``.
        - ``"log"``: undo the ``spec_factor`` scaling, then invert the
          ``log(1 + |spec|)`` compression via ``exp(|spec|) - 1``.
        - ``"none"`` or ``None``: leave the spectrum unchanged.

        Args:
            spec: compressed complex-valued spectrum.
        Returns:
            The spectrum with the compression undone, same shape/type as ``spec``.
        """
    ...
    def forward_streaming(self, input_frame: torch.Tensor) -> torch.Tensor:
        """Forward.
    ...
    def streaming_merge(
        self, chunks: List[torch.Tensor], ilens: Optional[torch.Tensor] = None
    ) -> torch.Tensor:
        ...
        # Overlap-add the windowed chunks.
        ...
        # Normalize by the overlap-added squared window ("window envelope"), which
        # is the standard correction for overlap-add synthesis with a windowed
        # analysis/synthesis pair (WOLA / Griffin-Lim style reconstruction).
