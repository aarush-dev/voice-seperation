"""Learned 1-D convolutional encoder (as used in e.g. Conv-TasNet)."""
...
class ConvEncoder(AbsEncoder):
    ...
    Applies a single strided 1-D convolution (with no bias) followed by a
    nonlinearity to the raw waveform, producing a learned filterbank
    representation analogous to an STFT magnitude spectrogram. This is the
    encoder used in Conv-TasNet-style time-domain separation models; the
    matching :class:`~espnet2.enh.decoder.conv_decoder.ConvDecoder` inverts
    it with a transposed convolution.
    """
    ...
    def forward(
        ...
        # (Batch, sample) -> (Batch, 1, sample): add the Conv1d input channel dim.
        input = torch.unsqueeze(input, 1)

        feature = self.conv1d(input)
        feature = self.activation(feature)
        # (Batch, channel, flens) -> (Batch, flens, channel)
    ...
    def forward_streaming(self, input: torch.Tensor) -> torch.Tensor:
        """Encode a single streaming frame using the same conv1d + activation."""
