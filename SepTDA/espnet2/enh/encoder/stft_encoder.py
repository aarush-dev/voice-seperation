"""STFT-based encoder: maps a waveform to a complex time-frequency spectrum."""
...
ComplexLike = Union[torch.Tensor, ComplexTensor]


class STFTEncoder(AbsEncoder):
    """STFT encoder for speech enhancement and separation.

    Wraps :class:`espnet2.layers.stft.Stft` to turn a waveform into a
    complex spectrogram, optionally applying a magnitude-compression
    transform ("spec_transform") on the resulting spectrum. The matching
    :class:`~espnet2.enh.decoder.stft_decoder.STFTDecoder` undoes both the
    inverse STFT and the spec transform.
    """
    ...
    def spec_transform_func(self, spec: ComplexLike) -> ComplexLike:
        """Apply the configured magnitude-compression transform to a spectrum.

        Rescales/compresses the spectrum magnitude while preserving phase,
        as used to stabilize training of generative speech-enhancement
        models. Controlled by ``self.spec_transform_type``:

        - ``"exponent"``: raise the magnitude to ``spec_abs_exponent`` (a
          power-law compression), then scale by ``spec_factor``.
        - ``"log"``: apply ``log(1 + |spec|)`` compression, then scale by
          ``spec_factor``.
        ...
        Returns:
            The transformed spectrum, same shape and type as ``spec``.
        """
    ...
    def forward(
        self, input: torch.Tensor, ilens: torch.Tensor, fs: int = None
    ) -> Tuple[ComplexLike, torch.Tensor]:
        """Forward.
    ...
    def _apply_window_func(self, input: torch.Tensor) -> torch.Tensor:
        """Window a single streaming frame and zero-pad it to length ``n_fft``."""
        ...
        windowed = torch.cat(
            [
                torch.zeros(batch_size, n_pad_left),
                windowed,
                torch.zeros(batch_size, n_pad_right),
            ],
            1,
        )
        return windowed

    def forward_streaming(self, input: torch.Tensor) -> ComplexLike:
        """Forward.
