...
"""Temporal Convolutional Network (TCN) mask-estimation network from Conv-TasNet.

Stacks dilated, depth-wise separable 1-D convolutions (with exponentially
growing dilation within each repeat) to estimate per-speaker time-frequency
masks from an encoder's bottleneck features. ``TemporalConvNetInformed`` is
a target-speaker variant that injects a speaker adaptation layer at a fixed
depth in the stack (see :mod:`espnet2.enh.layers.adapt_layers`).
"""
...
_MASK_NONLINEAR = {
    "softmax": lambda score, dim: torch.softmax(score, dim=dim),
    "relu": lambda score, dim: torch.relu(score),
    "sigmoid": lambda score, dim: torch.sigmoid(score),
    "tanh": lambda score, dim: torch.tanh(score),
    "linear": lambda score, dim: score,
}


def _apply_mask_nonlinear(score: Tensor, mask_nonlinear: str, dim: int = 1) -> Tensor:
    """Apply the configured nonlinearity that turns raw scores into a mask."""
    if mask_nonlinear not in _MASK_NONLINEAR:
        raise ValueError("Unsupported mask non-linear function")
    return _MASK_NONLINEAR[mask_nonlinear](score, dim)
...
class TemporalConvNet(nn.Module):
    ...
    def _run_tcn_stack(self, output: Tensor) -> Tuple[Tensor, Union[Tensor, float]]:
        """Run the repeated dilated-conv blocks with residual (+ skip) connections.

        Args:
            output: (M, B, K) bottleneck features.

        Returns:
            output: (M, B, K) final residual stream.
            skip_conn: (M, Sc, K) accumulated skip-connection sum, or 0.0 if
        ...
        return output, skip_conn

    def forward(self, mixture_w: Tensor) -> Tensor:
        ...
        output, skip_conn = self._run_tcn_stack(output)
        ...
        return _apply_mask_nonlinear(score, self.mask_nonlinear, dim=1)
...
class TemporalConvNetInformed(TemporalConvNet):
    ...
    def _run_tcn_stack(
        self, output: Tensor, enroll_emb: Tensor
    ) -> Tuple[Tensor, Union[Tensor, float]]:
        """Same as ``TemporalConvNet._run_tcn_stack``, but injects the speaker
        adaptation layer at block index ``self.i_adapt_layer``.

        Args:
            output: (M, B, K) bottleneck features.
            enroll_emb: (M, 2*adapt_enroll_dim) if skip connections are used,
                otherwise (M, adapt_enroll_dim).
        """
        tcns = self.network[2]
        skip_conn = 0.0
    ...
    def forward(self, mixture_w: Tensor, enroll_emb: Tensor) -> Tensor:
        ...
        masknet = self.network[3:]
        output = bottleneck(mixture_w)
        output, skip_conn = self._run_tcn_stack(output, enroll_emb)
...
def check_nonlinear(nolinear_type: str) -> None:
    if nolinear_type not in ["softmax", "relu"]:
        raise ValueError("Unsupported nonlinear type")


def choose_norm(
    norm_type: str, channel_size: int, shape: str = "BDT"
) -> nn.Module:
...
class ChannelwiseLayerNorm(nn.Module):
    ...
    def __init__(self, channel_size: int, shape: str = "BDT"):
        super().__init__()
