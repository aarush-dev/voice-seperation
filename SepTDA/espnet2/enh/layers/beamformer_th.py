...
This is a torchaudio-native counterpart of `beamformer.py`: it implements
(mostly) the same set of beamformer designs, but operates only on native
``torch`` complex tensors and delegates several core computations (RTF
estimation, Souden-form MVDR weights, PSD estimation) to
``torchaudio.functional``/``torchaudio.transforms`` instead of the
hand-written dual-backend routines in `beamformer.py`. See that module's
docstring for the beamformer math and shape notation (``psd``, ``Rf``, `B`,
`F`, `C`, `T`, `btaps`/`bdelay`), which apply identically here.
"""
...
def _as_double_mask_list(
    masks_speech: Union[torch.Tensor, Sequence[torch.Tensor]],
) -> List[torch.Tensor]:
    """Normalize `masks_speech` into a list of double-precision mask tensors."""
    if isinstance(masks_speech, (list, tuple)):
        return [m.cdouble() for m in masks_speech]
    return [masks_speech.cdouble()]
...
def _compute_rtf_style_psd_noise(
    signal: torch.Tensor,
    psd_transform,
    psd_speeches: List[torch.Tensor],
    ...
    See `beamformer._compute_rtf_style_psd_noise` for the full explanation;
    this is the torchaudio-backed equivalent (PSDs are computed with
    `psd_transform` instead of `get_power_spectral_density_matrix`).
...
def _select_psd_n(
    ...
    See `beamformer._select_psd_n` for the full explanation; behaves
    identically, using `torch.einsum` instead of the dual-backend `einsum`.
    """
...
def prepare_beamformer_stats(
    ...
    psd_noise = _compute_rtf_style_psd_noise(
        signal, psd_transform, psd_speeches, mask_noise, beamformer_type, num_spk
    )
...
def gev_phase_correction(vector: torch.Tensor) -> torch.Tensor:
...
def blind_analytic_normalization(
    ws: torch.Tensor, psd_noise: torch.Tensor, eps: float = 1e-8
) -> torch.Tensor:
...
def signal_framing(
    ...
    See `beamformer.signal_framing` for the frame layout used by WPD (each
    frame is `frame_length - 1` immediately preceding samples plus one extra
    sample delayed by `bdelay`).
...
def get_WPD_filter(
    ...
    numerator = torch.linalg.solve(Rf, Phi)  # Rf^-1 @ Phi
    # NOTE (wangyou): until PyTorch 1.9.0, torch.trace does not
    # support bacth processing. Use FC.trace() as fallback.
    # ws: (..., C, C) / (...,) -> (..., C, C)
...
def tik_reg(mat: torch.Tensor, reg: float = 1e-8, eps: float = 1e-8) -> torch.Tensor:
