...
class DenselyConnectedBlock(nn.Module):
    def __init__(
        ...
        # use (1, 4) to alleviate the checkerboard artifacts
        last_kernel_size: Tuple[int, int] = (1, 4),
        last_stride: Tuple[int, int] = (1, 2),
        last_padding: Tuple[int, int] = (0, 1),
        last_output_padding: Tuple[int, int] = (0, 0),
        layers: int = 5,
...
class DC_CRN(nn.Module):
    def __init__(
        ...
        T = 42
        hidden_sizes, hdim = self._build_encoder(
            input_dim,
            input_channels,
            enc_hid_channels,
            enc_kernel_size,
    ...
    def _build_encoder(
        ...
        enc_hid_channels: int,
        enc_kernel_size: Tuple[int, int],
        enc_padding: Tuple[int, int],
        enc_last_kernel_size: Tuple[int, int],
        enc_last_stride: Tuple[int, int],
        enc_last_padding: Tuple[int, int],
        enc_layers: int,
        T: int,
    ) -> Tuple[List[int], int]:
        """Build the stack of DenselyConnectedBlock encoder layers.

        Returns the frequency dimension after each encoder layer (including
        the input) and the final frequency dimension, both needed to size
        the GLSTM bottleneck and the decoder's skip/deconv pathways.
        """
    ...
    def _build_decoder(
        ...
        enc_last_padding: Tuple[int, int],
        enc_layers: int,
        skip_last_kernel_size: Tuple[int, int],
        skip_last_stride: Tuple[int, int],
        skip_last_padding: Tuple[int, int],
        hidden_sizes: List[int],
        T: int,
    ) -> None:
        """Build the skip-connection pathway and the deconvolution decoder.

        Mirrors the encoder in reverse, using ``hidden_sizes`` (recorded by
        ``_build_encoder``) to compute the exact output padding needed so
        each deconv layer's frequency dimension matches its skip connection.
        """
