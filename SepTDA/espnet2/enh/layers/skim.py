...
"""SkiM: Skipping Memory LSTM for low-latency streaming speech separation.

The input sequence is split into non-overlapping (or, if ``seg_overlap``,
50%-overlapping) segments. A "Seg-LSTM" (:class:`SegLSTM`) processes each
segment independently, while a "Mem-LSTM" (:class:`MemLSTM`) propagates the
Seg-LSTM's hidden/cell state across segments so that information can flow
between segments without ever unrolling an LSTM over the full sequence at
once. This keeps the per-step compute/latency bounded by the segment size,
which is what makes the model suitable for real-time streaming.
"""
...
HiddenCell = Tuple[Tensor, Tensor]
...
class MemLSTM(nn.Module):
    ...
    def forward(self, hc: HiddenCell, S: int) -> HiddenCell:
        """Propagate Seg-LSTM states across segments.

        Args:
            hc: (h, c), the stacked hidden and cell states from all segments
                of one SegLSTM. Each tensor has shape (d, B*S, H), where d is
                the number of directions.
            S: number of segments.

        Returns:
            The updated (h, c) with the same shape as the input, shifted by
            one segment (causal) when ``bidirectional`` is False, so that
            segment ``s`` only sees information from segments ``< s``.
        """
        if self.mem_type == "id":
            ret_val = hc
            h, c = hc
        ...
        if not self.bidirectional:
            # for causal setup, shift states forward by one segment so that
            # segment s can only see (memory of) segments < s
            causal_ret_val = []
            for x in ret_val:
    ...
    def forward_one_step(
        self, hc: HiddenCell, state: List[Optional[HiddenCell]]
    ) -> Tuple[HiddenCell, List[Optional[HiddenCell]]]:
        """Streaming counterpart of :meth:`forward`, updating one segment at a time.

        Args:
            hc: (h, c) from the just-finished segment, each of shape (d, B, H).
            state: the Mem-LSTM's own recurrent state (one entry per h_net/
                c_net), updated in place and also returned.
        """
        if self.mem_type == "id":
            pass
        else:
...
class SegLSTM(nn.Module):
    ...
    def forward(
        self, input: Tensor, hc: Optional[HiddenCell]
    ) -> Tuple[Tensor, HiddenCell]:
        """Run one segment through the LSTM, with a residual connection.

        Args:
            input: (B, T, H) one segment's worth of features.
            hc: (h, c) carried over from the previous segment (via MemLSTM),
                or None for the very first segment of a SkiM block.
        """
        B, T, H = input.shape
...
class SkiM(nn.Module):
    ...
    def forward(self, input: Tensor) -> Tensor:
        """Full-sequence (offline) forward pass.

        Args:
            input: (B, T, D) with T = S * K once padded/split into S
                segments of length K (``segment_size``).
    ...
    def _padfeature(self, input: Tensor) -> Tuple[Tensor, int]:
        """Zero-pad the time axis so it is an exact multiple of ``segment_size``."""
        B, T, D = input.shape
    ...
    def forward_stream(
        self, input_frame: Tensor, states: Optional[Dict]
    ) -> Tuple[Tensor, Dict]:
        """Streaming forward pass, consuming one time frame at a time.

        Args:
            input_frame: (B, 1, N) a single input frame.
            states: streaming state dict as produced by a previous call, or
                None to start a fresh stream. Holds the current position in
                the segment, each SegLSTM's (h, c), and each MemLSTM's own
                recurrent state.

        Returns:
            output: (B, 1, output_size)
            states: the updated streaming state dict.
        """
        B, _, N = input_frame.shape

        def empty_seg_states() -> HiddenCell:
        ...
        if states["current_step"] and (states["current_step"]) % self.segment_size == 0:
            # a segment just finished: refresh each block's starting SegLSTM
            # state from the MemLSTM before continuing to the next segment
