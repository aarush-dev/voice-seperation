...
class MaskEstimator(torch.nn.Module):
    """Estimates one or more time-frequency masks from (multi-channel) STFT features.

    A BLSTM (optionally projected/subsampled, see ``type``) processes each
    channel independently, and one linear layer per requested mask maps its
    output back to the frequency dimension, followed by a nonlinearity.
    """
    ...
    def forward(
        ...
        for linear in self.linears:
            ...
            mask = linear(xs)
            mask = self._activate_and_reformat_mask(mask, ilens, input_length)
            masks.append(mask)

        return tuple(masks), ilens

    def _activate_and_reformat_mask(
        self, mask: torch.Tensor, ilens: torch.LongTensor, input_length: int
    ) -> torch.Tensor:
        """Apply the mask nonlinearity, zero out padding, and restore (B, F, C, T).

        Args:
            mask: (B, C, T, F)
            ilens: (B,)
            input_length: original T before any multi-GPU truncation.
        """
