"""TCNDenseUNet from iNeuBe: a DenseNet U-Net encoder/decoder around a TCN bottleneck.
...
The frequency axis is progressively downsampled by a DenseNet-style U-Net
encoder, processed by a stack of dilated TCN residual blocks at the
bottleneck (where the frequency axis has been fully collapsed), and then
upsampled back by a mirrored decoder with U-Net skip connections.
"""
...
class Conv2DActNorm(torch.nn.Module):
    ...
    def __init__(
        ...
        in_channels: int,
        out_channels: int,
        ksz: Tuple[int, int] = (3, 3),
        stride: Tuple[int, int] = (1, 2),
        padding: Tuple[int, int] = (1, 0),
        upsample: bool = False,
        activation: Union[str, Type[torch.nn.Module]] = torch.nn.ELU,
    ):
...
class FreqWiseBlock(torch.nn.Module):
    ...
    def forward(self, inp: Tensor) -> Tensor:
        """(B, in_channels, frames, freq) -> (B, out_channels, frames, freq)."""
        # swap the channel and frequency axes so freq_proc's Conv2D mixes
        # information across frequency bins instead of channels
...
class DenseBlock(torch.nn.Module):
    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        Every sub-block's output is concatenated (densely) with all previous
        outputs (and the original input) before being fed to the next block.
        """
        out = [input]
...
class TCNDenseUNet(torch.nn.Module):
    ...
    def __init__(
        ...
        mic_channels: int = 1,
        hid_chans: int = 32,
        hid_chans_dense: int = 32,
        ksz_dense: Tuple[int, int] = (3, 3),
        ksz_tcn: int = 3,
        tcn_repeats: int = 4,
        tcn_blocks: int = 7,
        ...
        self._build_tcn(tcn_channels, ksz_tcn, tcn_repeats, tcn_blocks, activation)
        self._build_decoder(
            hid_chans,
            hid_chans_dense,
            ksz_dense,
    ...
    def _build_encoder(
        ...
        hid_chans: int,
        hid_chans_dense: int,
        ksz_dense: Tuple[int, int],
        tcn_channels: int,
        freq_axis_dims: List[int],
        activation: Union[str, Type[torch.nn.Module]],
    ) -> None:
        """Build the DenseNet U-Net encoder: progressively downsamples the
        frequency axis until a single frequency bin remains."""
        num_freqs = self.in_channels - 2
    ...
    def _build_tcn(
        ...
        """Build the bottleneck stack of dilated TCN residual blocks."""
        tcn_layers = []
    ...
    def _build_decoder(
        ...
        """Build the DenseNet U-Net decoder, mirroring the encoder and taking
        U-Net skip connections from each encoder stage (hence the doubled
        input channel counts below)."""
    ...
    def _get_depth(self, num_freq: int) -> List[int]:
        """Frequency-axis size after each halving downsample, stopping once
        it would drop to 15 or fewer bins."""
        freqs = []
        while num_freq > 15:
    ...
    def forward(self, tf_rep: Tensor) -> Union[Tensor, ComplexTensor]:
        """forward.
