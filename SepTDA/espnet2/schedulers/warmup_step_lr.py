...
class WarmupStepLR(_LRScheduler, AbsBatchStepScheduler):
    ...
    Selected via the "warmupsteplr" name in YAML training configs
    ...
    def __init__(
        ...
        Args:
            ...
            warmup_steps: Number of steps over which the learning rate
                ramps up before the step decay phase begins.
            steps_per_epoch: Number of training iterations (batches) per
                epoch. Used to translate step counts into epoch counts for
                the post-warmup step decay.

                NOTE: This number should be adjusted accordingly
                once batch_size/ngpu/num_nodes is changed. To get the exact
                number of iterations per epoch, refer to
                https://github.com/espnet/espnet/discussions/4404
            step_size: Number of epochs between each decay by `gamma`,
                applied after warmup.
    ...
    def get_lr(self) -> List[float]:
        ...
        During warmup (step_num <= warmup_steps), the learning rate ramps up
        linearly. After warmup, it decays by `gamma` every `step_size`
        epochs, counting epochs from the end of the warmup phase.
        """
        self.step_num += 1
