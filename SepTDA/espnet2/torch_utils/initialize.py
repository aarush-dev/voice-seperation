...
def initialize(model: torch.nn.Module, init: str) -> None:
    ...
    Args:
        model: Target.
        init: Method of initialization. One of "chainer", "xavier_uniform",
            "xavier_normal", "kaiming_uniform", "kaiming_normal".

    Raises:
        NotImplementedError: If `init == "chainer"` and a parameter has a
            number of dimensions the chainer-style scheme doesn't handle.
        ValueError: If `init` is not one of the recognized method names.
    """
    assert check_argument_types()

    if init == "chainer":
        _initialize_chainer_style(model)
    else:
        _initialize_torch_style(model, init)


def _initialize_chainer_style(model: torch.nn.Module) -> None:
    """Apply the "chainer"-compatible initialization scheme to `model`.

    Mirrors Chainer's default parameter initialization: LeCun-normal-like
    scaling for linear/conv weights, zeroed biases, unit-normal embedding
    weights, and forget-gate biases set to 1.0 for RNN cells.
    """
...
def _initialize_torch_style(model: torch.nn.Module, init: str) -> None:
    """Apply a standard torch.nn.init scheme (xavier/kaiming) to `model`.

    Args:
        model: Target module.
        init: One of "xavier_uniform", "xavier_normal", "kaiming_uniform",
            "kaiming_normal".
    ...
    _reload_pretrained_submodule_parameters(model)


def _reload_pretrained_submodule_parameters(model: torch.nn.Module) -> None:
    """Re-load pretrained weights for submodules that support it.

    Some submodules (e.g. s3prl frontend, wav2vec2 encoder) load their own
    pretrained weights via a `reload_pretrained_parameters()` method, which
    must be called *after* the generic re-initialization above overwrites
    them, otherwise those pretrained weights would be lost.
